...
from logic.user_logic import registation, get_profile, lvl_up, inventory_check, bal_top, lvl_top, is_register
...
async def cmd_profile(message: types.Message):
    ...
    await message.answer(prof, parse_mode='Markdown')
...
async def cmd_lvlup(message: types.Message):
    ...
    await message.answer(lvl, parse_mode='Markdown')
...
async def cmd_weather(message: types.Message):
    await message.answer(f'🌤️ Текущая погода: *{weather_manager.current.name}*\n\n🌱 множитель роста: *x{weather_manager.current.grow_multiplier}*\n⚡ бонус энергии: *{weather_manager.current.energy_bonus}*', parse_mode='Markdown')
...
async def cmd_baltop(message: types.Message):
    ...
    await message.answer(top, parse_mode='Markdown')
...
async def cmd_ref_start(message: types.Message, command: CommandObject=None):
    args = command.args if command else None
    user_id = message.from_user.id
    if not is_register(user_id):
        ref_id = None
        if args and args.isdigit() and int(args) != user_id:
            ...
            try:
                ...
                pass
        registr = await asyncio.to_thread(registation, name=message.from_user.first_name, username=message.from_user.username, user_id=user_id, referer_id=ref_id)
        await message.answer(f'{registr} Вы зарегестрировались по рефералочке')
...
async def cmd_get_ref_list(message: types.Message):
    await message.answer(f'Реферальная система\nВы ничего не зарабатываете приглашая рефералов\nВаша реферальная ссылка:\nt.me/sanyabaranovbot?start=ref{message.from_user.id}')